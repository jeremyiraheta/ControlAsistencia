...
router.post("/reporteHoras.pdf", (req, res) => {
    try {        
        ...
        (select CONCAT(LPAD(FLOOR(SUM(t.TOTAL) / 60),2,0), 'h',LPAD(SUM(t.TOTAL) % 60, 2, 0), 'm') from REGISTROS t where t.CODCLI = r.CODCLI and t.CODEMP = r.CODEMP and YEAR(t.FECHA) = YEAR(r.FECHA) AND MONTH(t.FECHA) = MONTH(r.FECHA)) horas, 
